...
npm run dev          # webpack-dev-server with hot reload at localhost:8080 (alias: npm start)
npm run build        # production build → outputs to docs/ (GitHub Pages)
npm run build --report   # build + webpack-bundle-analyzer report
...
- Main application (`src/`) — a Vue 2 app (scaffolded from the vuejs-templates/webpack template) where the user pastes text or picks a preset, and sees the unique words ranked by frequency and CEFR difficulty.
...
`static/10000.txt` (also copied to `docs/static/`) is the reference list of the 10,000 most common English words, ordered by frequency. A word's CEFR level (A1–C2) is derived purely from its **rank/index in this list** (see `level()` in `src/components/WordsList.vue` and the `etalon` "absolute index" in the CLI). When changing level thresholds, update both this list reference and the boundary constants. Note `static/` and `docs/static/` hold parallel copies — `docs/` is the built output, so source data edits go in `static/`.
...
- Per-document output goes to `--save` (or `.cache/<host>-<hash>.json`); summary metrics are also merged into the aggregate `static/list.json` stats DB.

## Conventions
- ESLint config is `eslint-config-standard` (`.eslintrc.js`); linting is **not** run during the webpack build (`config/index.js` → `useEslint: false`).
- `@` is a webpack alias for `src/` (see `build/webpack.base.conf.js`).
- The build deliberately targets `docs/` so the app is served from GitHub Pages; `assetsPublicPath` is relative (`./`) for production.
